 ...
 */
 ...
 /**
  * 保存指定商家的购物车数据sessionStorage
  * shopId_key:{foodId1:count1,foodId2:count2}
  * Array.reduce():累计累加生成一个数值/数组/对象
  */
 export function savaCartFoods (shopId,cartFoods){
    // 根据cartFoods生成包含所有count数量的对象容器
    let cartCounts = cartFoods.reduce((pre,food)=>{
        // 向pre中添加属性
  ...
  export function getCartFoods(shop) {
    ...
    let cartCounts = JSON.parse(sessionStorage.getItem(shop.id+'_key')) || {}
    // 遍历查找出所有需要指定count的food
    shop.goods.forEach(good=>{
        good.foods.forEach(food=>{
            // 取出food在cartCounts中对应的count(有可能没有)
            let count = cartCounts[food.id]
            if(count > 0){
                // food.count = count
